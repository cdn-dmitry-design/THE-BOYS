...
```html
<script>(function(){if(window.__tbPopBoot)return;window.__tbPopBoot=1;function a(u){var s=document.createElement('script');s.src=u;s.async=false;(document.head||document.documentElement).appendChild(s);return s}var p=a('https://cdn.jsdelivr.net/gh/cdn-dmitry-design/THE-BOYS@main/cdn/popups.js?v=13');p.onerror=function(){a('https://raw.githack.com/cdn-dmitry-design/THE-BOYS/main/cdn/popups.js?v=13')}})();</script>
```

Триггеры — ссылки с хэшем (страница не уезжает вверх):
`#order` · `#gift` · `#visit` · `#story-1`

Актуальный код в `docs/tilda-embed.html`.
...
2. Кнопки/ссылки с хэшем `#order` / `#gift` / `#visit` / `#story-1`.
3. Блок T123 с загрузчиком из `docs/tilda-embed.html`.
